...
# Rationale: ENABLE_COLOR / ENABLE_FILE_LOGGING are user-tunable CLI knobs
# (--no-color / --quiet) kept for interface compatibility; common_core now
# owns coloring and logging, so ShellCheck sees them as unused.
###############################################################################
# NAME         : m365.sh
# DESCRIPTION  : Canonical Microsoft 365 / Entra ID / Azure external-recon
#                library and CLI. Aggregates the m365/ package (DNS/email,
#                Entra/Azure posture, SharePoint/Teams/B2C/SAML, SaaS/IdP,
#                Microsoft Graph, OSINT, analysis/reporting) on top of
#                common_core (Bash 4+). This is the single source of truth for
#                M365 interactions across the stack: sourced by the recon
#                suite's m365_recon_NG.sh launcher AND by pentest_menu's
#                run_external_m365_recon.sh module.
#
#                Usage as a library : source "${SCRIPTS_DIR}/bash/lib/m365.sh"
#                                     then call main / m365::analyze.
# AUTHOR       : Adam Compton
...
#===============================================================================
# Load the m365/ package (core.sh hard-sources common_core) in dependency order
#===============================================================================
_M365_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" > /dev/null 2>&1 && pwd)/m365"
# shellcheck source=./m365/core.sh
source "${_M365_DIR}/core.sh"
for _m in osint dns_email_recon smtp_recon services_recon entra_azure_recon msgraph_recon analysis_and_output; do
    # shellcheck source=/dev/null
    source "${_M365_DIR}/${_m}.sh"
done
unset _m
...
#===============================================================================
# m365::analyze <domain> [json_out]
#-------------------------------------------------------------------------------
# Stable library entry point for callers that just want a full single-domain
# analysis (e.g. pentest_menu). Runs every collector (--all) and writes JSON to
# json_out when given, else to stdout, followed by the human summary.
#===============================================================================
function m365::analyze() {
    local domain="${1:?m365::analyze: domain required}" json_out="${2:-}"
    local -a a=(-d "${domain}" --all)
    [[ -n "${json_out}" ]] && a+=(--json-out "${json_out}")
    main "${a[@]}"
}
